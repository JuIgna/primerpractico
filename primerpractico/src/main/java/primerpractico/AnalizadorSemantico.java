...
// CODIGO SIN USO ACTUALMENTE QUE USAMOS PARA LA PRIMERA VERSION DEL TP2
...
class AnalizadorSemantico extends primerpractico.compiladoresBaseListener {
    ...
    @Override
    public void enterDeclaracion(primerpractico.compiladoresParser.DeclaracionContext ctx) {
    ...
    @Override
    public void enterParametros(primerpractico.compiladoresParser.ParametrosContext ctx) {
        for (primerpractico.compiladoresParser.ParametroContext param : ctx.parametro()) {
    ...
    @Override
    public void enterAsignacion(primerpractico.compiladoresParser.AsignacionContext ctx) {
        String nombre = ctx.ID().getText();
    ...
    @Override
    public void enterExpresion(primerpractico.compiladoresParser.ExpresionContext ctx) {
        if (ctx.ID() != null) {
            String nombre = ctx.ID().getText();
            tablaSimbolos.verificarSimbolo(nombre);
        }
    ...
    @Override
    public void exitPrograma(primerpractico.compiladoresParser.ProgramaContext ctx) {
        tablaSimbolos.imprimirSimbolos(escritorErrores);
    }

    private String getTipoExpresion(primerpractico.compiladoresParser.ExpresionContext ctx) {
        if (ctx.literal() != null) {
